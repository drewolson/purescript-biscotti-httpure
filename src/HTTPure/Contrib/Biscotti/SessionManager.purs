...
createSession' ::
  ...
  HTTPure.Response ->
  m (Either String HTTPure.Response)
createSession' store cookieUpdater session response =
  runExceptT do
...
destroySession ::
  ...
  HTTPure.Response ->
  m (Either String HTTPure.Response)
destroySession name store request response =
  runExceptT do
...
setSession' ::
  ...
  HTTPure.Response ->
  m (Either String HTTPure.Response)
setSession' name store cookieUpdater session request response =
  runExceptT do
    cookie <- except $ getSessionCookie name request
...
setSessionCookie response cookie = Lens.setJust (_headers <<< _Newtype <<< at (CaseInsensitiveString responseCookieTag)) (Cookie.stringify cookie) response
